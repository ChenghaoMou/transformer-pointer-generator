...
class EncoderDecoder(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask):
        # print(self.tgt_embed(tgt).shape, memory.shape, src_mask.shape, tgt_mask.shape)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        for layer in self.layers:
            ...
            attns.append(layer.src_attn.attn)
...
class CopyGenerator(nn.Module):
    ...
    def __init__(self, d_model, vocab):
        ...
        self.prob = nn.Linear(d_model * 3, 1)
        self.vocab = vocab

    def forward(self, dec_output, src_full=None, enc_output=None, dec_attns=None, dec_embeded=None):

        batch, steps, hidden = dec_output.size()
        ...
        if src_full is not None:
            logits = torch.mul(p_gen, logits)
            vocab_size = max(self.vocab, torch.max(src_full).item() + 1)
